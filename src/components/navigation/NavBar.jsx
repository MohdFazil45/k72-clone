...
const NavBar = () => {
  ...
  const isDarkPage = location.pathname === "/"; // home is dark
  const themeClass = isDarkPage ? "text-white" : "text-black";
  ...
  return (
    <div className=' flex fixed top-0 w-full items-start justify-between z-10'>
        <div className={`h-24 w-28 lg:w-36 p-2 ${themeClass}`}>
            <svg className='w-full font-extrabold' fill="currentColor" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 103 44">
        ...
        <div 
         ...
         onMouseLeave={() => {
          ...
          className="relative h-12 w-36 lg:h-12 lg:w-44  bg-black">
          <div ref={navGreenRef} className='bg-[#D3FD50] h-0 transition-all w-full absolute top-0 text-black'></div>
            <div className="relative h-full px-4 flex flex-col justify-center items-end gap-2">
              <div className="lg:w-16 w-20 h-0.5 bg-white"></div>
              <div className="lg:w-10 w-12 h-0.5 bg-white"></div> 
          </div>
        </div>
    </div>
  )
